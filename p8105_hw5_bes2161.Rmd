...
```{r}
plot_sim_results = sim_results_df %>% 
  mutate(
    flag = p.value < .05) %>% 
  group_by(flag) %>% 
  summarise(
    mean(flag)) %>% 
  ggplot(aes(x = flag, y = true_mean)) + 
    geom_violin() + 
    facet_grid(. ~ true_mean)

```

This plot shows the fraction of times the p.value is less than .05.
...
sim_results_df %>% 
  group_by(iter) %>%
  summarise(
    emp_mean = mean(p.value) %>% 
  ggplot(aes(x = p.value)) + 
  geom_bar()

```

No, the two means are not the same. When we have low power, the means are really different, and emphasized the need to be skeptical when reporting results.
